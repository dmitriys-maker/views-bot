...
# { user_id: { "platform": "TikTok", "channel": "МойКанал", "waiting_channel": False, "queue": [], ... } }
user_states = {}
...
def save_to_sheet(editor_name: str, channel: str, tg_id: int, platform: str, videos: list):
    ...
    for v in videos:
        rows_to_add.append([
            now, editor_name, channel, str(tg_id),
            platform, v["views"], len(videos), v.get("description", "")
        ])
...
def update_summary(ws_sum, editor_name, channel, new_views, new_vids, new_screens, now):
    ...
    if key in keys:
        ...
        ws_sum.update(f"A{row_num}:F{row_num}", [[
            editor_name, channel,
            old_views + new_views,
            old_vids + new_vids,
            old_screens + new_screens,
            now
        ]])
    else:
...
async def scan_image(image_bytes: bytes, mime_type: str = "image/jpeg") -> list:
            ...
            json={
                ...
                "messages": [{
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": (
                            "На скрине статистика видео (TikTok/YouTube/VK Видео). "
                            "Найди ВСЕ числа просмотров для КАЖДОГО видео на скрине. "
                            "Просмотры: 'просмотры', 'views', иконка глаза. "
                            "Конвертируй: 1.2K=1200, 1.5M=1500000, 2.3млн=2300000, 500тыс=500000. "
                            "Ответь ТОЛЬКО JSON без пояснений и markdown: "
                            "{\"videos\":[{\"views\":число,\"description\":\"название видео если видно\"}]}"
                        )}
                    ]
                }]
            }
...
async def process_queue(user_id: int, editor_name: str, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if state["session_screens"] > 0:
        await ctx.bot.send_message(
            ...
            f"Для новой сессии снова выбери платформу: /tiktok /youtube /vk"
        )
...
async def set_platform(update: Update, ctx: ContextTypes.DEFAULT_TYPE, platform: str):
...
async def set_tiktok(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await set_platform(update, ctx, "TikTok")

async def set_youtube(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await set_platform(update, ctx, "YouTube")

async def set_vk(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await set_platform(update, ctx, "VK Видео")
...
async def text_received(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if state["waiting_channel"]:
        ...
        await update.message.reply_text(
            f"Канал: {channel}\n"
            f"Платформа: {state['platform']} ✓\n\n"
            f"Теперь кидай скрины — обработаю все подряд!"
        )
    else:
        await update.message.reply_text(
            "Выбери платформу командой:\n"
            "/tiktok — TikTok\n"
            "/youtube — YouTube\n"
            "/vk — VK Видео"
        )
...
async def photo_received(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(state["queue"]) == 1 and not state["processing"]:
        await update.message.reply_text(
            f"Получил! Платформа: {state['platform']}, канал: {state['channel']}\n"
            f"Кидай остальные скрины, обработаю все по очереди."
        )
...
async def total_stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for name, data in sorted(editors.items(), key=lambda x: x[1]["views"], reverse=True):
            channels = ", ".join(set(data["channels"]))
            lines.append(f"• {name}: {fmt(data['views'])} просмотров ({data['vids']} роликов)\n  Каналы: {channels}")
